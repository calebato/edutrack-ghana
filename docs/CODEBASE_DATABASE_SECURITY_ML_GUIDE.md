...
| `database/` | Assessor-ready SQL, migrations, data audits, and curriculum maintenance scripts. |
...
| `database/edutrack_ghana.sql` | Complete privacy-safe schema and assessment data for fresh installations. |
...
| `database/seed_all_subject_topic_quizzes.php`, `database/seed_second_topic_quizzes.php` | Curriculum quiz maintenance utilities. |
| `database/seed_french_quizzes.php`, `database/seed_ghanaian_language_quizzes.php` | Language-subject quiz maintenance utilities. |

## Database Architecture

`database/edutrack_ghana.sql` contains the complete current schema and privacy-safe assessment records. A fresh setup imports this single file. SQL files in `database/migrations/` upgrade databases restored from older exports and should be applied in filename order only for those upgrade paths.
...
| Audit and security | `activity_logs`, `login_logs`, `system_logs` | Login/activity traces and admin actions. |
...
| `students` | unique `email`, `school_id`, `teacher_id`, `student_id`, `class_level`, `learning_style`, `difficulty_level`, parent contact fields | Main learner profile. Tracks points, streaks, activity, activation, parent contact, and password hash. |
...
| `questions` | `quiz_id`, four options, `correct_answer`, `difficulty`, `points` | MCQ bank used by active quizzes. |
| `quiz_attempts` | `student_id`, `quiz_id`, `score`, `answers_json`, `started_at`, `completed_at` | Stores submitted answers, score, timing, pass state, and attempt number. |
...
| `ml/train_model.py` | Trains a standard-library ridge linear regression model and writes the generated, gitignored artifact at `ml/models/exam_predictor.json`. |
...
- `config/db.php` reads environment-specific database credentials and retains local XAMPP defaults for development. Production must use HTTPS, restricted PHP error output, and a least-privilege database user.
- Personal forecasts require five completed quizzes (`EDUTRACK_PERSONAL_FORECAST_MINIMUM`). Keep user-facing copy, tests, and ML constants aligned if this policy changes.
- The repository SQL contains anonymized assessment records. Never replace it with an export containing real student, teacher, parent, or school-sensitive information.
